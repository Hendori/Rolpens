...

{
  ...
  if (iVar1 != 0) {
    iVar1 = qtest_create_quic_objects(0,pSVar2,0,cert,privkey,0,&local_48,&local_40,&local_38,0);
    iVar1 = test_true("test/quicfaultstest.c",0xbd,
                      "qtest_create_quic_objects(NULL, cctx, NULL, cert, privkey, 0, &qtserv, &cssl, &fault, NULL)"
                      ,iVar1 != 0);
    if (iVar1 != 0) {
      local_4c = (-(uint)(param_1 == 0) & 0x29) + 0x10;
      iVar1 = qtest_fault_set_hand_enc_ext_listener(local_38,drop_extensions_cb,&local_4c);
      iVar1 = test_true("test/quicfaultstest.c",0xc9,
                        "qtest_fault_set_hand_enc_ext_listener(fault, drop_extensions_cb, &ext)",
                        iVar1 != 0);
      if (iVar1 != 0) {
        iVar1 = qtest_create_quic_connection(local_48,local_40);
        ...
        if (iVar1 != 0) {
          iVar1 = qtest_check_server_transport_err
                            (local_48,(-(ulong)(param_1 == 0) & 0xfffffffffffffff5) + 0x178);
  ...
  bVar3 = false;
LAB_001004ff:
  qtest_fault_free(local_38);
  SSL_free(local_40);
  ossl_quic_tserver_free(local_48);
  SSL_CTX_free(pSVar2);
  if (local_30 == *(long *)(in_FS_OFFSET + 0x28)) {
    return bVar3;
  }
