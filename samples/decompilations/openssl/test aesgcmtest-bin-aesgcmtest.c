...
LAB_00100659:
    ...
    iVar1 = test_true("test/aesgcmtest.c",0x30,
                      "EVP_EncryptInit_ex(ctx, EVP_aes_256_gcm(), NULL, NULL, NULL) > 0",0 < iVar1);
    if (iVar1 == 0) goto LAB_00100659;
    iVar1 = EVP_EncryptInit_ex(ctx,(EVP_CIPHER *)0x0,(ENGINE *)0x0,gcm_key,gcm_iv);
    iVar1 = test_true("test/aesgcmtest.c",0x32,
                      "EVP_EncryptInit_ex(ctx, NULL, NULL, gcm_key, iv_gen != NULL ? NULL : gcm_iv) > 0"
                      ,0 < iVar1);
    if (iVar1 == 0) goto LAB_00100659;
    iVar1 = EVP_EncryptUpdate(ctx,(uchar *)0x0,&local_bc,gcm_aad,0x10);
    iVar1 = test_true("test/aesgcmtest.c",0x34,
                      "EVP_EncryptUpdate(ctx, NULL, &outlen, gcm_aad, sizeof(gcm_aad)) > 0",
                      0 < iVar1);
    if (iVar1 == 0) goto LAB_00100659;
    iVar1 = EVP_EncryptUpdate(ctx,local_98,&local_c0,gcm_pt,0x10);
    iVar1 = test_true("test/aesgcmtest.c",0x36,
                      "EVP_EncryptUpdate(ctx, ct, ct_len, gcm_pt, sizeof(gcm_pt)) > 0",0 < iVar1);
    if (iVar1 == 0) goto LAB_00100659;
    iVar1 = EVP_EncryptFinal_ex(ctx,local_78,&local_bc);
    iVar1 = test_true("test/aesgcmtest.c",0x38,"EVP_EncryptFinal_ex(ctx, outbuf, &outlen) > 0",
                      0 < iVar1);
    if (iVar1 == 0) goto LAB_00100659;
    uVar2 = EVP_CIPHER_CTX_get_tag_length();
    iVar1 = test_int_eq("test/aesgcmtest.c",0x39,"EVP_CIPHER_CTX_get_tag_length(ctx)",&_LC4,uVar2,
                        0x10);
    if (iVar1 == 0) goto LAB_00100659;
    iVar1 = EVP_CIPHER_CTX_ctrl(ctx,0x10,0x10,local_b8);
    iVar1 = test_true("test/aesgcmtest.c",0x3a,
                      "EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, 16, tag) > 0",0 < iVar1);
    if ((iVar1 == 0) ||
       (iVar1 = test_true("test/aesgcmtest.c",0x3c,
                          "iv_gen == NULL || EVP_CIPHER_CTX_get_original_iv(ctx, iv_gen, 12)",1),
       iVar1 == 0)) goto LAB_00100659;
    EVP_CIPHER_CTX_free(ctx);
    iVar1 = test_mem_eq("test/aesgcmtest.c",100,"gcm_ct",&_LC23,gcm_ct,0x10,local_98,(long)local_c0)
    ;
    if ((iVar1 != 0) &&
       (iVar1 = test_mem_eq("test/aesgcmtest.c",0x65,"gcm_tag",&_LC25,gcm_tag,0x10,local_b8,0x10),
       iVar1 != 0)) {
      iVar1 = do_decrypt(gcm_iv,local_98,local_c0,local_b8,0x10);
      bVar3 = iVar1 != 0;
      goto LAB_00100660;
    }
  }
