  ...
  if (param_1 == 0) {
    if (2 < opt_verbosity) {
      BIO_printf(_bio_err,"%s:%s:%d:CMP %s: NULL arg to write_PKIMESSAGE%s%s%s\n","write_PKIMESSAGE"
                 ,"apps/cmp.c",0x32d,"error",&_LC4,&_LC4,&_LC4);
      return 0;
    }
  }
  else {
  ...
  if (lVar1 == 0) {
    ...
    *param_2 = lVar2;
    ...
    if (lVar2 != 0) {
      ...
      BIO_printf(_bio_out,"%s:%s:%d:CMP %s: %s %s%s\n","read_PKIMESSAGE","apps/cmp.c",0x354,&_LC9,
                 param_1,lVar1,&_LC4);
      return lVar2;
    }
    if (2 < opt_verbosity) {
...
LAB_001004a1:
    ...
    if (opt_rspin == 0) {
      ...
      if (((opt_reqin_new_tid != 0) &&
         ...
         (iVar2 = OSSL_CMP_MSG_update_recipNonce(param_1,param_2), iVar2 == 0)) goto LAB_001004f2;
...

{
  ...
  if (*pcVar1 == '\0') {
    ...
    BIO_printf(_bio_out,"%s:%s:%d:CMP %s: %s option argument starts with hyphen%s%s\n","opt_str",
               "apps/cmp.c",0xae6,"warning",uVar2,&_LC4,&_LC4);
  }
  return pcVar1;
}
...
uint save_cert_or_delete(long param_1,char *param_2,undefined8 param_3)
...
undefined8 get_opts(undefined8 param_1,undefined8 param_2)
...
switchD_00100ed2_caseD_0:
  do {
    ...
    uVar1 = opt_verbosity;
    if (iVar2 == 0) {
switchD_00100ed2_caseD_ffffffff:
LAB_00100f2f:
...
LAB_001019a6:
    if ((opt_srv_secret != 0) || ((opt_srv_cert == 0) == (opt_srv_key == 0))) {
      if (opt_srv_cert != 0) {
LAB_001019c7:
        lVar10 = opt_srv_cert;
      ...
      if (opt_srv_key != 0) {
        ...
        pEVar9 = (EVP_PKEY *)
                 load_key(lVar10,uVar1,0,uVar4,param_1,"private key for mock server cert");
        clear_free(uVar4);
        if ((pEVar9 == (EVP_PKEY *)0x0) ||
           (iVar2 = OSSL_CMP_CTX_set1_pkey(uVar3,pEVar9), iVar2 == 0)) {
      ...
      if (opt_srv_trusted == 0) {
        ...
        if ((v == (X509_STORE *)0x0) ||
           (iVar2 = OSSL_CMP_CTX_set0_trustedStore(uVar3,v), iVar2 == 0)) {
          X509_STORE_free(v);
          goto LAB_00101fb0;
        }
      }
      if (opt_srv_untrusted != 0) {
        lVar10 = load_certs_multifile
                           (opt_srv_untrusted,opt_otherpass,"untrusted certificates for mock server"
                            ,vpm);
      ...
      if (opt_rsp_key != 0) {
        ...
        pEVar9 = (EVP_PKEY *)
                 load_key(lVar10,uVar1,0,uVar4,param_1,"private key for enrollment cert");
        clear_free(uVar4);
        if ((pEVar9 == (EVP_PKEY *)0x0) ||
      ...
      if (opt_rsp_extracerts != 0) {
        lVar10 = load_certs_multifile
                           (opt_rsp_extracerts,opt_otherpass,
                            "CMP extra certificates for mock server",vpm);
        if (lVar10 == 0) goto LAB_00101fb0;
      ...
      if (opt_rsp_newwithnew != 0) {
        uVar4 = get_passwd(opt_otherpass,
...
LAB_001022a0:
        if (0x7ffffff < opt_failurebits) {
          if (2 < opt_verbosity) {
            BIO_printf(_bio_err,"%s:%s:%d:CMP %s: -failurebits out of range%s%s%s\n","setup_srv_ctx"
                       ,"apps/cmp.c",0x4f8,"error",&_LC4,&_LC4,&_LC4);
          }
          goto LAB_00101fb0;
        }
      }
        ...
        if (0x1a < opt_failure) {
          if (2 < opt_verbosity) {
            BIO_printf(_bio_err,
                       "%s:%s:%d:CMP %s: -failure out of range, should be >= 0 and <= %d%s%s\n",
...
undefined8
setup_request_ctx(undefined8 param_1,undefined8 param_2,undefined8 param_3,char *param_4,
...

{
  ...
  if ((((opt_subject != (X509_REQ *)0x0) || (opt_csr != 0)) || (opt_oldcert != 0)) ||
     (uVar6 = 0, opt_cert != 0)) {
LAB_0010244b:
    if (opt_issuer == 0) {
LAB_00102494:
      if (2 < opt_cmd) goto LAB_00102740;
              ...
              if (opt_key == 0) {
                if (2 < opt_verbosity) {
                  BIO_printf(_bio_err,
                             ...
                             ,"setup_request_ctx","apps/cmp.c",0x6ca,"error",&_LC4,&_LC4,&_LC4,
                             (ulong)opt_popo);
                }
                goto LAB_001026c7;
              }
...
LAB_00102fd2:
            if (-1 < (int)uVar6) {
              if (2 < opt_verbosity) {
                BIO_printf(_bio_err,
                           "%s:%s:%d:CMP %s: -popo value %d is inconsistent with -centralkeygen%s%s\n"
...
LAB_001024cc:
        if (opt_newkey == 0) goto LAB_00102b58;
      }
joined_r0x001024e2:
...
LAB_00102994:
      ...
      BIO_printf(_bio_out,"%s:%s:%d:CMP %s: Ignoring -serial for command other than \'rr\'%s%s%s\n",
                 "setup_request_ctx");
LAB_001025cd:
      if (opt_cmd != 3) goto LAB_001025d6;
    }
    if (opt_csr == 0) {
      if (2 < opt_verbosity) {
...
LAB_00102cc3:
    if ((opt_newkeypass != (X509_REQ *)0x0) && (3 < opt_verbosity)) {
      param_4 = "apps/cmp.c";
...
LAB_00102cf5:
      if (opt_popo != 0xfffffffe) goto LAB_00102690;
    }
...
LAB_00103786:
          if (opt_serial == (char *)0x0) goto LAB_001025b0;
        }
...
LAB_00102674:
        iVar4 = OSSL_CMP_CTX_set0_newPkey(param_1,uVar16,pEVar8);
        if (iVar4 != 0) goto LAB_00102690;
      }
      EVP_PKEY_free(pEVar8);
      goto LAB_001026c7;
    }
    if ((((opt_reqin != (byte *)0x0) && (opt_key == 0)) && (opt_csr == 0)) &&
      ...
      if (bVar17 != 0x2c) {
        ...
        pbVar18 = pbVar1;
        do {
          if (((*(byte *)((long)*ppuVar9 + (ulong)bVar17 * 2 + 1) & 0x20) != 0) || (bVar17 == 0))
          break;
          bVar17 = pbVar18[1];
          pbVar18 = pbVar18 + 1;
        } while (bVar17 != 0x2c);
      ...
      *pbVar18 = bVar17;
      ...
      if ((lVar12 == 0) || (lVar12 = X509_PUBKEY_get0(lVar12), lVar12 == 0)) {
        if (2 < opt_verbosity) {
          BIO_printf(_bio_err,
...
LAB_00102690:
  if ((0 < opt_days) && (iVar4 = OSSL_CMP_CTX_set_option(param_1), iVar4 == 0)) {
    if (2 < opt_verbosity) {
...
LAB_00103827:
    if (2 < opt_verbosity) {
      BIO_printf(_bio_err,"%s:%s:%d:CMP %s: out of memory%s%s%s\n","setup_request_ctx","apps/cmp.c",
                 0x7d6,"error",&_LC4,&_LC4,&_LC4);
    }
LAB_00103834:
    OPENSSL_sk_pop_free(local_90._0_8_,&X509_EXTENSION_free);
    X509_REQ_free(pXVar13);
    goto LAB_001026c7;
  }
LAB_00102d96:
  pXVar13 = (X509_REQ *)0x0;
LAB_00102d98:
  ...
  if (iVar4 == 0) {
    ...
    do {
      ...
      if (iVar4 == 0) {
        ...
        ERR_set_mark();
        param_4 = (char *)0x7;
        a_01 = a2i_GENERAL_NAME((GENERAL_NAME *)0x0,(X509V3_EXT_METHOD *)0x0,(X509V3_CTX *)0x0,7,
                                pcVar19,0);
        if (a_01 == (GENERAL_NAME *)0x0) {
          ...
          a_01 = a2i_GENERAL_NAME((GENERAL_NAME *)0x0,(X509V3_EXT_METHOD *)0x0,(X509V3_CTX *)0x0,
                                  (int)param_4,pcVar19,0);
          ERR_pop_to_mark();
          if (a_01 == (GENERAL_NAME *)0x0) {
            if (2 < opt_verbosity) {
              BIO_printf(_bio_err,"%s:%s:%d:CMP %s: bad syntax of %s \'%s\'%s\n","set_gennames",
                         "apps/cmp.c",0x3e6,"error","Subject Alternative Name",pcVar19,&_LC4);
            }
            goto LAB_001026c7;
          }
        }
    ...
    if (opt_policy_oids == (char *)0x0) {
      ...
      if (opt_oldcert != 0) {
        if (opt_cmd == 5) {
          ...
          if ((opt_cmd != 1) && (pcVar19 = "reference certificate (oldcert)", opt_cmd == 4)) {
...
int setup_client_ctx(long param_1,ulong param_2)

{
  ...
  if (opt_use_mock_srv == 0) {
    ...
    puVar27 = (ushort *)&_LC4;
LAB_001054c8:
    BIO_printf(_bio_err,pcVar23,pcVar20,"apps/cmp.c",uVar25,"error",puVar27,&_LC4,&_LC4);
    goto LAB_0010477c;
  }
LAB_00104697:
  if (opt_server == (ushort *)0x0) {
LAB_001048d0:
    if (opt_proxy == 0) {
LAB_0010492f:
...
LAB_0010498c:
    ...
    if (((bVar2 != 0x69) || (*(byte *)((long)opt_cmd_s + 1) != 0x72)) ||
       (uVar6 = (uint)(byte)opt_cmd_s[1], (byte)opt_cmd_s[1] != 0)) {
      pEVar29 = (EVP_PKEY *)0x105507;
      iVar5 = strcmp((char *)opt_cmd_s,"kur");
      if (iVar5 == 0) {
        uVar6 = 1;
      }
      else if (((bVar2 != 99) || (*(byte *)((long)puVar27 + 1) != 0x72)) ||
              (uVar6 = 2, (byte)puVar27[1] != 0)) {
        pEVar29 = (EVP_PKEY *)0x10553f;
        iVar5 = strcmp((char *)puVar27,"p10cr");
        if (iVar5 == 0) {
          uVar6 = 3;
        }
        else if (((bVar2 != 0x72) || (*(byte *)((long)puVar27 + 1) != 0x72)) ||
                (uVar6 = 4, (byte)puVar27[1] != 0)) {
...
LAB_00104a80:
      if ((opt_oldwithold != 0) && (3 < opt_verbosity)) {
        BIO_printf(_bio_out,"%s:%s:%d:CMP %s: -oldwithold %s%s%s\n","setup_client_ctx","apps/cmp.c",
                   0x8b9,"warning",
                   "option is ignored unless -cmd \'genm\' and -infotype rootCaCert is given",&_LC4,
                   &_LC4);
      }
      if (opt_newwithnew == 0) {
LAB_001056b7:
...
LAB_00104ac0:
      if ((opt_cmd != 5) || (opt_infotype != 0x4c9)) goto LAB_00104acd;
...
LAB_00105b19:
      if (opt_cmd != 5) goto LAB_00104a80;
LAB_00105896:
      if (opt_infotype != 0x4e6) goto LAB_00104a80;
LAB_00104acd:
      if (opt_template != 0) {
...
LAB_00104c3a:
    pAVar18 = (ASN1_UTF8STRING *)(ulong)opt_unprotected_errors;
    ...
    if (opt_out_trusted != 0) {
      pXVar11 = (X509_STORE *)
                load_certstore(opt_out_trusted,opt_otherpass,
                               "trusted certs for verifying newly enrolled cert",vpm);
...
LAB_00105c38:
      if (opt_reqin != 0) goto LAB_00104d66;
LAB_0010595c:
...
LAB_0010527f:
        ...
        if (opt_key != 0) {
          ...
          if ((pEVar14 == (EVP_PKEY *)0x0) ||
             (pcVar23 = (char *)pEVar14, iVar5 = OSSL_CMP_CTX_set1_pkey(param_1), iVar5 == 0)) {
            iVar5 = 0;
...
LAB_001051ef:
      ...
      if (opt_cert == 0) {
        if ((opt_own_trusted != (X509_STORE *)0x0) && (3 < opt_verbosity)) {
          ...
          pcVar20 = "warning";
          BIO_printf(_bio_out,
                     "%s:%s:%d:CMP %s: -own_trusted option is ignored without -cert%s%s%s\n",
                     "setup_protection_ctx","apps/cmp.c",0x647,"warning",&_LC4,&_LC4,&_LC4);
        }
LAB_00105fe0:
        if (opt_extracerts != 0) {
          lVar9 = load_certs_multifile
                            (opt_extracerts,opt_otherpass,"extra certificates for CMP",vpm);
          if (lVar9 == 0) goto LAB_00104774;
...
LAB_00106bf4:
                if (2 < opt_verbosity) {
                  BIO_printf(_bio_err,"%s:%s:%d:CMP %s: out of memory%s%s%s\n","handle_opt_geninfo",
                             "apps/cmp.c",0x850,"error",&_LC4,&_LC4,&_LC4,pcVar23);
                }
LAB_00106bfd:
                ASN1_OBJECT_free(o);
                iVar5 = 0;
              ...
              if (iVar5 == 0) {
                    ...
                    if ((byte)*local_250 != 0x2c) {
                      if (2 < opt_verbosity) {
                        BIO_printf(_bio_err,
                                   ...
                                   ,"handle_opt_geninfo","apps/cmp.c",0x821,"error",local_250,&_LC4,
                                   &_LC4,pcVar20);
                      }
                      goto LAB_00106d23;
                    }
                ...
                if (2 < opt_verbosity) {
                  BIO_printf(_bio_err,
                             ...
                             "handle_opt_geninfo","apps/cmp.c",0x81b,"error",puVar27,&_LC4,&_LC4,
                             pAVar18);
                }
LAB_00106d23:
                pAVar18 = (ASN1_UTF8STRING *)0x0;
              ...
              if (iVar5 != 0) {
                if (2 < opt_verbosity) {
                  BIO_printf(_bio_err,
                             ...
                             ,"handle_opt_geninfo","apps/cmp.c",0x83c,"error",pcVar1,&_LC4,&_LC4,
                             uVar26);
                }
                goto LAB_00106d23;
              }
              ...
              if (lVar9 == 0) {
                if (2 < opt_verbosity) {
                  BIO_printf(_bio_err,
                             ...
                             ,"handle_opt_geninfo","apps/cmp.c",0x841,"error",&_LC4,&_LC4,&_LC4,
                             pAVar24);
                }
                goto LAB_00106d23;
              }
              ...
              if (iVar5 == 0) {
                ...
                if (2 < opt_verbosity) {
                  BIO_printf(_bio_err,
                             ...
                             ,"handle_opt_geninfo","apps/cmp.c",0x848,"error",&_LC4,&_LC4,&_LC4,
                             lVar19);
                }
...
LAB_00106895:
          if ((opt_reqout_only == 0) && (5 < opt_verbosity)) {
            ...
            BIO_printf(_bio_out,"%s:%s:%d:CMP %s: will contact %s%s%s \n","setup_client_ctx",
                       "apps/cmp.c",0x91f,&_LC9,local_1d8,local_108,pcVar20,&_LC4);
          }
          else {
            iVar5 = 1;
          }
        ...
        if ((param_1 == 0) || (lVar9 = OPENSSL_sk_new_reserve(0,1), lVar9 == 0)) goto LAB_00104774;
        pAVar18 = ASN1_UTF8STRING_new();
          ...
          if (iVar5 == 0) {
            if (2 < opt_verbosity) {
              BIO_printf(_bio_err,"%s:%s:%d:CMP %s: out of memory%s%s%s\n","setup_protection_ctx",
                         "apps/cmp.c",0x639,"error",&_LC4,&_LC4,&_LC4,extraout_RAX_00);
...
LAB_0010537e:
            ...
            if ((opt_own_trusted != (X509_STORE *)0x0) &&
               ...
               pXVar11 == (X509_STORE *)0x0)) goto LAB_0010537e;
...
LAB_00104e73:
              ...
              iVar5 = load_cert_certs(opt_tls_cert,&local_258,&local_250,0,opt_tls_keypass,
                                      "TLS client certificate (optionally with chain)",vpm);
              if (iVar5 != 0) {
                iVar5 = SSL_CTX_use_certificate(ctx,local_258);
                X509_free(local_258);
                lVar9 = extraout_RAX;
                if ((iVar5 < 1) || (lVar9 = SSL_CTX_ctrl(ctx,0x58,0,local_250), lVar9 == 0)) {
                  if (2 < opt_verbosity) {
                    pcVar20 = "error";
                    uVar26 = 0x587;
                    BIO_printf(_bio_err,
                               "%s:%s:%d:CMP %s: unable to use client TLS certificate file \'%s\'%s%s\n"
                               ,"setup_ssl_ctx","apps/cmp.c",0x587,"error",opt_tls_cert,&_LC4,&_LC4,
                               lVar9);
                  }
                  OSSL_STACK_OF_X509_free(local_250);
                }
                  ...
                  if (lVar9 == 0) {
                    ...
                    if (3 < opt_verbosity) {
                      ...
                      BIO_printf(_bio_out,
                                 ...
                                 ,"setup_ssl_ctx","apps/cmp.c",0x5a9,"warning",&_LC4,&_LC4,&_LC4,
                                 in_R11);
                    }
                  }
...
LAB_00105085:
                      ...
                      if (iVar5 == 0) {
                        if (2 < opt_verbosity) {
                          ...
                          BIO_printf(_bio_err,
                                     ...
                                     ,"setup_ssl_ctx","apps/cmp.c",0x5d3,"error",opt_tls_key,
                                     opt_tls_cert,&_LC4,pAVar18);
                        }
                      }
                      else {
                        ...
                        if (2 < opt_verbosity) {
                          ...
                          BIO_printf(_bio_err,
                                     ...
                                     ,"setup_ssl_ctx","apps/cmp.c",0x5da,"error",opt_tls_key,&_LC4,
                                     &_LC4,pcVar20);
                          pcVar20 = pcVar23;
                        }
                      }
                    ...
                    if (lVar9 != 0) {
                      iVar5 = 1;
                      for (iVar7 = 0; iVar8 = OPENSSL_sk_num(lVar9), iVar7 < iVar8;
                          iVar7 = iVar7 + 1) {
...
LAB_00105124:
            if (local_298 == (X509_STORE *)0x0) goto LAB_0010514e;
          }
...
LAB_001046a0:
    iVar5 = OSSL_HTTP_parse_url(opt_server,&local_274,0,&local_270,&local_268,&local_278,&local_260,
                                0,0);
    if (iVar5 == 0) {
      iVar5 = 0;
      if (opt_verbosity < 3) goto LAB_0010477c;
    ...
    if ((((iVar5 != 0) && (iVar5 = OSSL_CMP_CTX_set_serverPort(param_1,local_278), iVar5 != 0)) &&
        ((opt_proxy == 0 || (iVar5 = OSSL_CMP_CTX_set1_proxy(param_1), iVar5 != 0)))) &&
       ((opt_no_proxy == 0 || (iVar5 = OSSL_CMP_CTX_set1_no_proxy(param_1), iVar5 != 0)))) {
      pcVar23 = pcVar20 + 1;
      if (*pcVar20 != '/') {
        pcVar23 = pcVar20;
      }
...
bool cmp_main(int param_1,BIO **param_2,undefined8 param_3,undefined8 param_4,char *param_5,
             char *param_6)
...
LAB_00107040:
    ...
    if (vpm == (X509_VERIFY_PARAM *)0x0) {
      ...
      if (opt_config == (BIO *)0x0) {
        if ((_default_config_file != (BIO *)0x0) &&
           (pBVar21 = _default_config_file, *(char *)&_default_config_file->method != '\0')) {
LAB_00107908:
          iVar29 = access((char *)pBVar21,0);
          ppBVar40 = local_130;
...
LAB_001070a0:
        ...
        if (iVar29 != 0) {
          ...
          do {
            if ((puVar11 == puVar20) ||
               (puVar11 = (undefined *)prev_item_part_0(puVar20,puVar11),
               puVar11 == (undefined *)0x0)) goto LAB_0010714f;
...
LAB_0010714f:
        ...
        do {
          ...
          if ((iVar3 == 0) || (iVar3 = strcmp(pcVar42,&OPT_MORE_STR), iVar3 == 0)) {
            iVar29 = iVar29 + -1;
          }
        ...
        if (iVar29 != 0x9d) {
          auVar44 = OPENSSL_die("assertion failed: OSSL_NELEM(cmp_vars) == n_options + OPT_PROV__FIRST + 1 - OPT_PROV__LAST + OPT_R__FIRST + 1 - OPT_R__LAST + OPT_V__FIRST + 1 - OPT_V__LAST"
                                ,"apps/cmp.c",0xa6d);
          puVar34 = auStack_138;
        ...
        do {
          ...
          if ((iVar4 != 0) &&
             (iVar4 = strcmp(pcVar41,&OPT_MORE_STR), puVar20 = opt_section, pCVar8 = conf,
             iVar4 != 0)) {
...
LAB_001077a5:
              ...
              if (((*pcVar28 != '\0') &&
                  (ppcVar15 = (char **)strtol(pcVar28,&local_c0,10),
                  (long)ppcVar15 + 0x7fffffffffffffffU < 0xfffffffffffffffe)) && (*local_c0 == '\0')
                 ) {
                if (*(int *)((long)local_130 + 0xc) == 0x70) {
                  ...
                  if ((*(int *)((long)local_130 + 0xc) != 0x4e) || (-1 < (long)ppcVar15))
                  goto LAB_00107890;
...
LAB_00108210:
                  ...
                  do {
                    if ((puVar11 == puVar20) ||
                       (puVar20 = (undefined *)prev_item_part_0(puVar11,puVar20),
...
LAB_001081b4:
                opt_init(uVar13,&local_b8,cmp_options);
                pXVar2 = vpm;
                if ((uint)local_118 < 5) {
                  uVar7 = opt_next();
                ...
                if (iVar3 == 0) {
                  if (2 < (int)opt_verbosity) {
                    BIO_printf(_bio_err,
                               ...
                               ,"read_config","apps/cmp.c",0xabb,"error",*local_130,opt_section,
                               &_LC4,uVar13);
                  }
                  goto joined_r0x00107b8d;
                }
...
LAB_001073a0:
      ...
      if (iVar29 != 0) {
        ...
        if ((opt_engine == (BIO **)0x0) ||
           (pcVar42 = (char *)setup_engine_methods(opt_engine,0,0), (BIO **)pcVar42 != (BIO **)0x0))
        {
...
LAB_00107957:
          ...
          else if (((((opt_tls_cert == 0) && (opt_tls_key == 0)) && (opt_tls_keypass == 0)) &&
              ...
              if ((opt_reqin == 0) && (opt_reqout == 0)) {
                ...
                if (2 < (int)opt_verbosity) {
                  BIO_printf(_bio_err,
                             ...
                             ,"cmp_main","apps/cmp.c",0xe80,"error",&_LC4,&_LC4,&_LC4,
                             (ulong)opt_use_mock_srv);
                  puVar32 = auStack_138;
                }
              }
              else {
...
LAB_00107bc0:
              ...
              if (opt_reqout_only == 0) {
                if ((opt_use_mock_srv == 0) && (opt_port == (code *)0x0)) {
                  ...
                  if ((opt_tls_used != 0) && (pcVar41 = opt_server, opt_server == (char *)0x0))
                  goto LAB_00107c0c;
                  goto LAB_00107c33;
                }
...
LAB_00107bf0:
                in_R11 = (ulong)opt_tls_used;
                if ((opt_tls_used != 0) &&
                   ((ppBVar40 = (BIO **)(ulong)opt_use_mock_srv, opt_use_mock_srv != 0 ||
                    (opt_server == (char *)0x0)))) {
LAB_00107c0c:
                  if (3 < (int)opt_verbosity) {
                    param_6 = "warning";
...
LAB_001087ad:
                    ...
                    if ((opt_server != (char *)0x0) && (3 < (int)opt_verbosity)) {
                      ...
                      BIO_printf(_bio_out,"%s:%s:%d:CMP %s: -server %s%s%s\n","cmp_main",
                                 "apps/cmp.c",0xeb4,"warning",
                                 "option is ignored since -reqout_only option is given",&_LC4,&_LC4,
                                 pcVar35);
                    }
                    if ((opt_use_mock_srv != 0) && (3 < (int)opt_verbosity)) {
                      param_6 = "warning";
...
LAB_00107cbb:
                  iVar29 = setup_client_ctx(cmp_ctx,pcVar42);
                  if (iVar29 != 0) {
                    if (0 < opt_repeat) {
...
LAB_00107cf2:
                      ...
                      auVar44._0_8_ =
                           (long)&switchD_00107d1a::switchdataD_0011218c +
                           (long)(int)(&switchD_00107d1a::switchdataD_0011218c)[opt_cmd];
                      switch(opt_cmd) {
                      case 0:
                        uVar13 = 0;
                        break;
                        ...
                        if (x != (EVP_PKEY *)0x0) {
                          *(undefined8 *)(puVar34 + -8) = 0x108591;
                          ...
                          if (opt_newkeypass != (EVP_CIPHER *)0x0) {
                            *(undefined8 *)(puVar34 + -8) = 0x1085ba;
                            pcVar41 = (char *)get_passwd(opt_newkeypass,
                          ...
                          if (5 < (int)opt_verbosity) {
                            ...
                            *(undefined8 *)(puVar34 + -0x28) = 0x108ed3;
                            BIO_printf(_bio_out,
                                       "%s:%s:%d:CMP %s: saving centrally generated key to file \'%s\'%s%s\n"
                                       ,"cmp_main");
                          }
                          param_6 = (char *)0x0;
                          ...
                          *(undefined8 *)(puVar34 + -0x18) = 0x108607;
                          iVar29 = PEM_write_bio_PrivateKey
                                             (pBVar21,x,enc,(uchar *)0x0,0,(undefined1 *)0x0,
                                              *(void **)(puVar34 + -0x10));
                          if (0 < iVar29) {
...
LAB_00108b5c:
                  ...
                  if (pBVar21 != (BIO *)0x0) {
                    ...
                    do {
                      ...
                      pBVar39 = pBVar21;
                      iVar3 = http_server_get_asn1_req
                                        (uVar14,local_120,&local_c8,local_128,pBVar21,local_130,
                                         uVar13,0,0);
                      pcVar28 = local_c8;
                      if (iVar3 == 0) {
...
LAB_00108cfc:
                          ...
                          uVar5 = http_server_send_asn1_resp
                                            (prog,local_d0,local_d8[0],"application/pkixcmp",uVar13,
                                             local_118);
                          OSSL_CMP_MSG_free(local_118);
                          if (uVar5 == 0) goto LAB_00108cfc;
                          if ((local_d8[0] != 0) &&
                             (iVar3 = OSSL_CMP_CTX_get_status(pcVar41), iVar3 == -2))
                          goto LAB_00108925;
                        }
...
LAB_001076b6:
        ...
        if (2 < (int)opt_verbosity) {
          BIO_printf(_bio_err,"%s:%s:%d:CMP %s: cannot load engine %s%s%s\n","cmp_main","apps/cmp.c"
                     ,0xe5e,"error",opt_engine,&_LC4,&_LC4);
        }
      }
    }
  }
