...
{
  long lVar1;
  NoteGnuProperty_4 *pNVar2;
  
  pNVar2 = (NoteGnuProperty_4 *)&rstat_tbl_2;
  do {
    lVar1._0_4_ = pNVar2->namesz;
    lVar1._4_4_ = pNVar2->descsz;
    if (s == lVar1) {
      return *(char **)&pNVar2->type;
    }
    pNVar2 = pNVar2 + 1;
  } while (pNVar2 != &NoteGnuProperty_4_00100be0);
  return "(UNKNOWN)";
}
...

{
  ...
  do {
    if (s == *plVar1) {
      return (char *)plVar1[1];
    }
    plVar1 = plVar1 + 2;
  } while (plVar1 != (long *)&DAT_00100b70);
  return "(UNKNOWN)";
}
