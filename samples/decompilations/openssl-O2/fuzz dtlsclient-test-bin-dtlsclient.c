  ...
  if ((ctx == (SSL_CTX *)0x0) || (ssl = SSL_new(ctx), ssl == (SSL *)0x0)) {
    ...
    if (iVar1 != 1) {
      OPENSSL_die("assertion failed: SSL_set_cipher_list(client, \"ALL:eNULL:@SECLEVEL=0\") == 1",
                  "fuzz/dtlsclient.c",0x4b);
LAB_00100237:
      OPENSSL_die("assertion failed: (size_t)BIO_write(in, buf, len) == len","fuzz/dtlsclient.c",
                  0x57);
LAB_0010024f:
      OPENSSL_die("assertion failed: SSL_set_min_proto_version(client, 0) == 1","fuzz/dtlsclient.c",
                  0x4a);
