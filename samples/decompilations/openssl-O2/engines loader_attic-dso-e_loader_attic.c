
undefined8 file_expect(long param_1,undefined4 param_2)

{
  *(undefined4 *)(param_1 + 0x40) = param_2;
  return 1;
}



bool file_error(long param_1)

{
  return 0 < *(int *)(param_1 + 0xc);
}



undefined8 loader_attic_init(void)

{
  return 1;
}



undefined8 loader_attic_destroy(void)

{
  long lVar1;
  undefined8 uVar2;
  
  lVar1 = OSSL_STORE_unregister_loader(&_LC0);
  uVar2 = 0;
  if (lVar1 != 0) {
    if (error_loaded == 0) {
      OSSL_STORE_LOADER_free(lVar1);
      uVar2 = 1;
    }
    else {
      ERR_unload_strings(lib_code,(ERR_STRING_DATA *)ATTIC_str_reasons);
      error_loaded = 0;
      OSSL_STORE_LOADER_free(lVar1);
      uVar2 = 1;
    }
  }
  return uVar2;
}



ulong check_suffix(char *param_1,char *param_2)

{
  int iVar1;
  size_t sVar2;
  size_t sVar3;
  ulong uVar4;
  
  sVar2 = strlen(param_1);
  sVar3 = strlen(param_2);
  iVar1 = (int)sVar3 + 1;
  if (iVar1 < (int)sVar2) {
    uVar4 = (long)(int)sVar2 - (long)iVar1;
    if (param_1[uVar4] == ' ') {
      iVar1 = strcmp(param_1 + uVar4 + 1,param_2);
      if (iVar1 != 0) {
        return 0xffffffff;
      }
      return uVar4 & 0xffffffff;
    }
  }
  return 0xffffffff;
}



long try_decode_PrivateKey
               (char *param_1,undefined8 param_2,uchar *param_3,long param_4,undefined8 param_5,
               int *param_6)

{
  int iVar1;
  int iVar2;
  EVP_PKEY_ASN1_METHOD *pEVar3;
  long lVar4;
  PKCS8_PRIV_KEY_INFO *a;
  ENGINE *e;
  ENGINE_PKEY_ASN1_METHS_PTR pEVar5;
  EVP_PKEY *pEVar6;
  long in_FS_OFFSET;
  undefined8 in_stack_00000020;
  undefined8 in_stack_00000028;
  EVP_PKEY *local_80;
  uchar *local_70 [2];
  int local_60;
  byte local_5c [4];
  int *local_58;
  EVP_PKEY_ASN1_METHOD *local_50;
  uchar *local_48;
  long local_40;
  
  local_40 = *(long *)(in_FS_OFFSET + 0x28);
  local_70[0] = param_3;
  if (param_1 == (char *)0x0) {
    e = ENGINE_get_first();
    local_80 = (EVP_PKEY *)0x0;
    if (e != (ENGINE *)0x0) {
      do {
        pEVar5 = ENGINE_get_pkey_asn1_meths(e);
        if (pEVar5 != (ENGINE_PKEY_ASN1_METHS_PTR)0x0) {
          local_58 = (int *)0x0;
          iVar1 = (*pEVar5)(e,(EVP_PKEY_ASN1_METHOD **)0x0,&local_58,0);
          if (0 < iVar1) {
            lVar4 = 0;
            do {
              local_50 = (EVP_PKEY_ASN1_METHOD *)0x0;
              local_48 = local_70[0];
              iVar2 = (*pEVar5)(e,&local_50,(int **)0x0,*(int *)((long)local_58 + lVar4));
              if (((iVar2 != 0) &&
                  (iVar2 = EVP_PKEY_asn1_get0_info
                                     (&local_60,(int *)0x0,(int *)local_5c,(char **)0x0,(char **)0x0
                                      ,local_50), iVar2 != 0)) && ((local_5c[0] & 1) == 0)) {
                ERR_set_mark();
                pEVar6 = (EVP_PKEY *)
                         d2i_PrivateKey_ex(local_60,0,&local_48,param_4,in_stack_00000020,
                                           in_stack_00000028);
                if (pEVar6 != (EVP_PKEY *)0x0) {
                  if (local_80 != (EVP_PKEY *)0x0) {
                    EVP_PKEY_free(pEVar6);
                    pEVar6 = local_80;
                  }
                  local_80 = pEVar6;
                  *param_6 = *param_6 + 1;
                }
                ERR_pop_to_mark();
              }
              lVar4 = lVar4 + 4;
            } while ((long)iVar1 * 4 - lVar4 != 0);
          }
        }
        e = ENGINE_get_next(e);
      } while (e != (ENGINE *)0x0);
    }
    for (iVar1 = 0; iVar2 = EVP_PKEY_asn1_get_count(), iVar1 < iVar2; iVar1 = iVar1 + 1) {
      local_48 = local_70[0];
      pEVar3 = EVP_PKEY_asn1_get0(iVar1);
      iVar2 = EVP_PKEY_asn1_get0_info
                        ((int *)&local_58,(int *)0x0,(int *)&local_50,(char **)0x0,(char **)0x0,
                         pEVar3);
      if ((iVar2 != 0) && (((ulong)local_50 & 1) == 0)) {
        ERR_set_mark();
        pEVar6 = (EVP_PKEY *)
                 d2i_PrivateKey_ex((ulong)local_58 & 0xffffffff,0,&local_48,param_4,
                                   in_stack_00000020,in_stack_00000028);
        if (pEVar6 != (EVP_PKEY *)0x0) {
          if (local_80 != (EVP_PKEY *)0x0) {
            EVP_PKEY_free(pEVar6);
            pEVar6 = local_80;
          }
          *param_6 = *param_6 + 1;
          local_80 = pEVar6;
        }
        ERR_pop_to_mark();
      }
    }
    if (1 < *param_6) {
      EVP_PKEY_free(local_80);
      lVar4 = 0;
      goto LAB_001001f7;
    }
LAB_001001e1:
    if (local_80 != (EVP_PKEY *)0x0) {
      lVar4 = OSSL_STORE_INFO_new_PKEY(local_80);
      if (lVar4 == 0) {
        EVP_PKEY_free(local_80);
      }
      goto LAB_001001f7;
    }
  }
  else {
    iVar1 = strcmp(param_1,"PRIVATE KEY");
    if (iVar1 == 0) {
      a = d2i_PKCS8_PRIV_KEY_INFO((PKCS8_PRIV_KEY_INFO **)0x0,local_70,param_4);
      *param_6 = 1;
      if (a != (PKCS8_PRIV_KEY_INFO *)0x0) {
        local_80 = (EVP_PKEY *)EVP_PKCS82PKEY_ex(a,in_stack_00000020,in_stack_00000028);
        PKCS8_PRIV_KEY_INFO_free(a);
        goto LAB_001001e1;
      }
      PKCS8_PRIV_KEY_INFO_free((PKCS8_PRIV_KEY_INFO *)0x0);
    }
    else {
      iVar1 = check_suffix(param_1,"PRIVATE KEY");
      if (((0 < iVar1) &&
          (pEVar3 = EVP_PKEY_asn1_find_str((ENGINE **)0x0,param_1,iVar1),
          pEVar3 != (EVP_PKEY_ASN1_METHOD *)0x0)) &&
         (iVar1 = EVP_PKEY_asn1_get0_info
                            ((int *)&local_48,(int *)0x0,(int *)0x0,(char **)0x0,(char **)0x0,pEVar3
                            ), iVar1 != 0)) {
        *param_6 = 1;
        local_80 = (EVP_PKEY *)
                   d2i_PrivateKey_ex((ulong)local_48 & 0xffffffff,0,local_70,param_4,
                                     in_stack_00000020,in_stack_00000028);
        goto LAB_001001e1;
      }
    }
  }
  lVar4 = 0;
LAB_001001f7:
  if (local_40 != *(long *)(in_FS_OFFSET + 0x28)) {
                    /* WARNING: Subroutine does not return */
    __stack_chk_fail();
  }
  return lVar4;
}



undefined8
try_decode_PUBKEY(char *param_1,undefined8 param_2,uchar *param_3,long param_4,undefined8 param_5,
                 undefined4 *param_6)

{
  int iVar1;
  EVP_PKEY *pEVar2;
  undefined8 uVar3;
  uchar *local_20 [2];
  
  local_20[0] = param_3;
  if (param_1 != (char *)0x0) {
    iVar1 = strcmp(param_1,"PUBLIC KEY");
    if (iVar1 != 0) {
      return 0;
    }
    *param_6 = 1;
  }
  pEVar2 = d2i_PUBKEY((EVP_PKEY **)0x0,local_20,param_4);
  if (pEVar2 == (EVP_PKEY *)0x0) {
    return 0;
  }
  *param_6 = 1;
  uVar3 = OSSL_STORE_INFO_new_PUBKEY(pEVar2);
  return uVar3;
}



long try_decode_params(char *param_1,undefined8 param_2,ulong param_3,undefined8 param_4,
                      undefined8 param_5,int *param_6)

{
  int iVar1;
  int iVar2;
  EVP_PKEY_ASN1_METHOD *pEVar3;
  EVP_PKEY *pkey;
  long lVar4;
  EVP_PKEY *pkey_00;
  long in_FS_OFFSET;
  ulong local_50 [2];
  int local_40;
  byte local_3c [4];
  ulong local_38;
  long local_30;
  
  local_30 = *(long *)(in_FS_OFFSET + 0x28);
  local_50[0] = param_3;
  if (param_1 == (char *)0x0) {
    pkey = (EVP_PKEY *)0x0;
    for (iVar1 = 0; iVar2 = EVP_PKEY_asn1_get_count(), iVar1 < iVar2; iVar1 = iVar1 + 1) {
      local_38 = local_50[0];
      pEVar3 = EVP_PKEY_asn1_get0(iVar1);
      iVar2 = EVP_PKEY_asn1_get0_info
                        (&local_40,(int *)0x0,(int *)local_3c,(char **)0x0,(char **)0x0,pEVar3);
      if ((iVar2 != 0) && ((local_3c[0] & 1) == 0)) {
        ERR_set_mark();
        pkey_00 = (EVP_PKEY *)d2i_KeyParams(local_40,0,&local_38);
        if (pkey_00 != (EVP_PKEY *)0x0) {
          if (pkey != (EVP_PKEY *)0x0) {
            EVP_PKEY_free(pkey_00);
            pkey_00 = pkey;
          }
          *param_6 = *param_6 + 1;
          pkey = pkey_00;
        }
        ERR_pop_to_mark();
      }
    }
    if (*param_6 < 2) goto LAB_001005ac;
    EVP_PKEY_free(pkey);
  }
  else {
    iVar1 = check_suffix(param_1,"PARAMETERS");
    if (((0 < iVar1) &&
        (pEVar3 = EVP_PKEY_asn1_find_str((ENGINE **)0x0,param_1,iVar1),
        pEVar3 != (EVP_PKEY_ASN1_METHOD *)0x0)) &&
       (iVar1 = EVP_PKEY_asn1_get0_info
                          ((int *)&local_38,(int *)0x0,(int *)0x0,(char **)0x0,(char **)0x0,pEVar3),
       iVar1 != 0)) {
      *param_6 = 1;
      pkey = (EVP_PKEY *)d2i_KeyParams(local_38 & 0xffffffff,0,local_50,param_4);
LAB_001005ac:
      if (pkey != (EVP_PKEY *)0x0) {
        lVar4 = OSSL_STORE_INFO_new_PARAMS(pkey);
        if (lVar4 == 0) {
          EVP_PKEY_free(pkey);
        }
        goto LAB_001005c6;
      }
    }
  }
  lVar4 = 0;
LAB_001005c6:
  if (local_30 != *(long *)(in_FS_OFFSET + 0x28)) {
                    /* WARNING: Subroutine does not return */
    __stack_chk_fail();
  }
  return lVar4;
}



long try_decode_X509CRL(char *param_1,undefined8 param_2,uchar *param_3,long param_4,
                       undefined8 param_5,undefined4 *param_6)

{
  int iVar1;
  X509_CRL *a;
  long lVar2;
  uchar *local_20 [2];
  
  local_20[0] = param_3;
  if (param_1 != (char *)0x0) {
    iVar1 = strcmp(param_1,"X509 CRL");
    if (iVar1 != 0) {
      return 0;
    }
    *param_6 = 1;
  }
  a = d2i_X509_CRL((X509_CRL **)0x0,local_20,param_4);
  if (a != (X509_CRL *)0x0) {
    *param_6 = 1;
    lVar2 = OSSL_STORE_INFO_new_CRL(a);
    if (lVar2 != 0) {
      return lVar2;
    }
  }
  X509_CRL_free(a);
  return 0;
}



long try_decode_X509Certificate
               (char *param_1,undefined8 param_2,uchar *param_3,long param_4,undefined8 param_5,
               undefined4 *param_6)

{
  int iVar1;
  int iVar2;
  X509 *pXVar3;
  long lVar4;
  long in_FS_OFFSET;
  undefined8 in_stack_00000020;
  undefined8 in_stack_00000028;
  uchar *local_50;
  X509 *local_48;
  long local_40;
  
  local_40 = *(long *)(in_FS_OFFSET + 0x28);
  local_48 = (X509 *)0x0;
  local_50 = param_3;
  if (param_1 == (char *)0x0) {
    iVar1 = 1;
LAB_001007b9:
    local_48 = (X509 *)X509_new_ex(in_stack_00000020,in_stack_00000028);
    if (local_48 != (X509 *)0x0) {
      pXVar3 = d2i_X509_AUX(&local_48,&local_50,param_4);
      if (pXVar3 == (X509 *)0x0) {
        if (iVar1 != 0) {
          pXVar3 = d2i_X509(&local_48,&local_50,param_4);
          if (pXVar3 != (X509 *)0x0) goto LAB_001007ef;
        }
      }
      else {
LAB_001007ef:
        *param_6 = 1;
        lVar4 = OSSL_STORE_INFO_new_CERT(local_48);
        if (lVar4 != 0) goto LAB_00100809;
      }
      X509_free(local_48);
      lVar4 = 0;
      goto LAB_00100809;
    }
  }
  else {
    iVar1 = strcmp(param_1,"TRUSTED CERTIFICATE");
    if (iVar1 == 0) {
LAB_001007b2:
      *param_6 = 1;
      goto LAB_001007b9;
    }
    iVar1 = strcmp(param_1,"X509 CERTIFICATE");
    if (iVar1 == 0) {
      iVar1 = 1;
      goto LAB_001007b2;
    }
    iVar1 = 1;
    iVar2 = strcmp(param_1,"CERTIFICATE");
    if (iVar2 == 0) goto LAB_001007b2;
  }
  lVar4 = 0;
LAB_00100809:
  if (local_40 == *(long *)(in_FS_OFFSET + 0x28)) {
    return lVar4;
  }
                    /* WARNING: Subroutine does not return */
  __stack_chk_fail();
}



void destroy_ctx_PKCS12(undefined8 *param_1)

{
  OPENSSL_sk_pop_free(*param_1,store_info_free);
  *param_1 = 0;
  return;
}



bool eof_PKCS12(long param_1)

{
  int iVar1;
  
  if (param_1 != 0) {
    iVar1 = OPENSSL_sk_num();
    return iVar1 == 0;
  }
  return true;
}



void store_info_free(long param_1)

{
  undefined8 *ptr;
  
  if (param_1 != 0) {
    ptr = (undefined8 *)OSSL_STORE_INFO_get0_data(0xffffffff,param_1);
    if (ptr != (undefined8 *)0x0) {
      BUF_MEM_free((BUF_MEM *)*ptr);
      CRYPTO_free((void *)ptr[1]);
      CRYPTO_free(ptr);
    }
  }
  OSSL_STORE_INFO_free(param_1);
  return;
}



undefined8 file_find_type(long param_1)

{
  BIO_METHOD *type;
  BIO *pBVar1;
  char *pcVar2;
  undefined8 uVar3;
  long lVar4;
  long in_FS_OFFSET;
  char acStack_1028 [4095];
  undefined1 local_29;
  long local_20;
  
  local_20 = *(long *)(in_FS_OFFSET + 0x28);
  pcVar2 = acStack_1028;
  for (lVar4 = 0x200; lVar4 != 0; lVar4 = lVar4 + -1) {
    *(undefined8 *)pcVar2 = 0;
    pcVar2 = (char *)((long)pcVar2 + 8);
  }
  type = BIO_f_buffer();
  pBVar1 = BIO_new(type);
  uVar3 = 0;
  if (pBVar1 != (BIO *)0x0) {
    pBVar1 = BIO_push(pBVar1,*(BIO **)(param_1 + 0x18));
    *(BIO **)(param_1 + 0x18) = pBVar1;
    lVar4 = BIO_ctrl(pBVar1,0x1d,0xfff,acStack_1028);
    if (0 < lVar4) {
      local_29 = 0;
      pcVar2 = strstr(acStack_1028,"-----BEGIN ");
      if (pcVar2 != (char *)0x0) {
        *(undefined4 *)(param_1 + 8) = 1;
      }
    }
    uVar3 = 1;
  }
  if (local_20 == *(long *)(in_FS_OFFSET + 0x28)) {
    return uVar3;
  }
                    /* WARNING: Subroutine does not return */
  __stack_chk_fail();
}



void OSSL_STORE_LOADER_CTX_free_part_0(undefined8 *param_1)

{
  CRYPTO_free((void *)param_1[10]);
  CRYPTO_free((void *)*param_1);
  if ((*(int *)(param_1 + 1) != 2) && (param_1[4] != 0)) {
    (**(code **)(param_1[4] + 0x18))(param_1 + 5);
    *(undefined1 (*) [16])(param_1 + 4) = (undefined1  [16])0x0;
  }
  CRYPTO_free(param_1);
  return;
}



undefined8 file_close(long param_1)

{
  BIO *b;
  
  if ((*(byte *)(param_1 + 0x10) & 2) == 0) {
    if (*(int *)(param_1 + 8) == 2) {
      OPENSSL_DIR_end(param_1 + 0x18);
    }
    else {
      BIO_free_all(*(BIO **)(param_1 + 0x18));
    }
  }
  else {
    b = *(BIO **)(param_1 + 0x18);
    BIO_pop(b);
    *(undefined8 *)(param_1 + 0x18) = 0;
    BIO_free(b);
  }
  OSSL_STORE_LOADER_CTX_free_part_0(param_1);
  return 1;
}



long file_attach(undefined8 param_1,undefined8 param_2,undefined8 param_3,char *param_4)

{
  int iVar1;
  long lVar2;
  char *pcVar3;
  
  lVar2 = CRYPTO_zalloc(0x58,"engines/e_loader_attic.c",0x432);
  if (lVar2 == 0) {
    return 0;
  }
  if (param_4 != (char *)0x0) {
    pcVar3 = CRYPTO_strdup(param_4,"engines/e_loader_attic.c",0x433);
    *(char **)(lVar2 + 0x50) = pcVar3;
    if (pcVar3 == (char *)0x0) goto LAB_00100bb8;
  }
  *(uint *)(lVar2 + 0x10) = *(uint *)(lVar2 + 0x10) | 2;
  *(undefined8 *)(lVar2 + 0x48) = param_3;
  *(undefined8 *)(lVar2 + 0x18) = param_2;
  iVar1 = file_find_type(lVar2);
  if (iVar1 != 0) {
    return lVar2;
  }
  *(undefined8 *)(lVar2 + 0x18) = 0;
LAB_00100bb8:
  OSSL_STORE_LOADER_CTX_free_part_0(lVar2);
  return 0;
}



void ERR_ATTIC_error_constprop_0(undefined4 param_1,undefined4 param_2)

{
  if (lib_code == 0) {
    lib_code = ERR_get_next_error_library();
  }
  ERR_new();
  ERR_set_debug("engines/e_loader_attic_err.c",0x47,"ERR_ATTIC_error");
  ERR_set_error(lib_code,param_1,0);
  ERR_set_debug("engines/e_loader_attic.c",param_2,0);
  return;
}



undefined8 * file_open_ex(undefined8 param_1,char *param_2,undefined8 param_3,char *param_4)

{
  int iVar1;
  long lVar2;
  undefined8 *puVar3;
  char *pcVar4;
  BIO *a;
  char *pcVar5;
  int *piVar6;
  undefined8 *puVar7;
  long lVar8;
  long in_FS_OFFSET;
  char *local_f8;
  byte local_f0 [24];
  stat local_d8;
  long local_40;
  
  local_40 = *(long *)(in_FS_OFFSET + 0x28);
  local_f0[0] = local_f0[0] & 0xfe;
  local_f8 = param_2;
  iVar1 = OPENSSL_strncasecmp(param_2,"file:",5);
  if (iVar1 == 0) {
    pcVar5 = param_2 + 5;
    if ((param_2[5] == '/') && (param_2[6] == '/')) {
      pcVar5 = param_2 + 7;
      iVar1 = OPENSSL_strncasecmp(pcVar5,"localhost/",10);
      if (iVar1 == 0) {
        pcVar5 = param_2 + 0x10;
      }
      else if (param_2[7] != '/') {
        puVar7 = (undefined8 *)0x0;
        ERR_ATTIC_error_constprop_0(0x6e,0x3d1);
        goto LAB_00100dbb;
      }
      lVar8 = 1;
      lVar2 = 0;
    }
    else {
      lVar8 = 2;
      lVar2 = 1;
    }
    local_f0[lVar2 * 0x10] = local_f0[lVar2 * 0x10] | 1;
    *(char **)(local_f0 + lVar2 * 0x10 + -8) = pcVar5;
  }
  else {
    lVar8 = 1;
  }
  lVar2 = 0;
  do {
    if (((local_f0[lVar2 * 0x10] & 1) != 0) &&
       (pcVar5 = *(char **)(local_f0 + lVar2 * 0x10 + -8), *pcVar5 != '/')) {
      puVar7 = (undefined8 *)0x0;
      ERR_ATTIC_error_constprop_0(0x69,0x3ed);
      ERR_add_error_data(1,pcVar5);
      goto LAB_00100dbb;
    }
    pcVar5 = *(char **)(local_f0 + lVar2 * 0x10 + -8);
    iVar1 = stat(pcVar5,&local_d8);
    if (-1 < iVar1) {
      puVar3 = (undefined8 *)CRYPTO_zalloc(0x58,"engines/e_loader_attic.c",0x400);
      if (puVar3 != (undefined8 *)0x0) {
        pcVar4 = CRYPTO_strdup(param_2,"engines/e_loader_attic.c",0x403);
        *puVar3 = pcVar4;
        if (pcVar4 != (char *)0x0) {
          if ((local_d8.st_mode & 0xf000) == 0x4000) {
            *(undefined4 *)(puVar3 + 1) = 2;
            lVar8 = OPENSSL_DIR_read(puVar3 + 3,pcVar5);
            puVar3[6] = lVar8;
            piVar6 = __errno_location();
            iVar1 = *piVar6;
            *(int *)(puVar3 + 7) = iVar1;
            if (lVar8 == 0) {
              if (iVar1 != 0) {
                ERR_new();
                ERR_set_debug("engines/e_loader_attic.c",0x40d,"file_open_ex");
                ERR_set_error(2,*(undefined4 *)(puVar3 + 7),0);
                goto LAB_00100ef1;
              }
              *(undefined4 *)(puVar3 + 4) = 1;
            }
LAB_00100d8d:
            if (param_4 != (char *)0x0) {
              pcVar5 = CRYPTO_strdup(param_4,"engines/e_loader_attic.c",0x418);
              puVar3[10] = pcVar5;
              if (pcVar5 == (char *)0x0) goto LAB_00100ef1;
            }
            puVar3[9] = param_3;
            puVar7 = puVar3;
            goto LAB_00100dbb;
          }
          a = BIO_new_file(pcVar5,"rb");
          puVar3[3] = a;
          if (a != (BIO *)0x0) {
            iVar1 = file_find_type(puVar3);
            if (iVar1 != 0) goto LAB_00100d8d;
            a = (BIO *)puVar3[3];
          }
          BIO_free_all(a);
        }
LAB_00100ef1:
        puVar7 = (undefined8 *)0x0;
        OSSL_STORE_LOADER_CTX_free_part_0(puVar3);
        goto LAB_00100dbb;
      }
      break;
    }
    ERR_new();
    ERR_set_debug("engines/e_loader_attic.c",0x3f3,"file_open_ex");
    lVar2 = lVar2 + 1;
    piVar6 = __errno_location();
    ERR_set_error(2,*piVar6,"calling stat(%s)",pcVar5);
  } while (lVar2 != lVar8);
  puVar7 = (undefined8 *)0x0;
LAB_00100dbb:
  if (local_40 == *(long *)(in_FS_OFFSET + 0x28)) {
    return puVar7;
  }
                    /* WARNING: Subroutine does not return */
  __stack_chk_fail();
}



void file_open(undefined8 param_1,undefined8 param_2,undefined8 param_3,undefined8 param_4)

{
  file_open_ex(param_1,param_2,0,0,param_3,param_4);
  return;
}



undefined8 file_ctrl(long param_1,int param_2,uint *param_3)

{
  uint uVar1;
  int iVar2;
  undefined8 *puVar3;
  
  if (param_2 != 1) {
    return 1;
  }
  uVar1 = *param_3;
  if (uVar1 < 0x30) {
    *param_3 = uVar1 + 8;
    iVar2 = **(int **)((ulong)uVar1 + *(long *)(param_3 + 4));
  }
  else {
    puVar3 = *(undefined8 **)(param_3 + 2);
    *(undefined8 **)(param_3 + 2) = puVar3 + 1;
    iVar2 = *(int *)*puVar3;
  }
  if (iVar2 != 0) {
    if (iVar2 == 1) {
      *(uint *)(param_1 + 0x10) = *(uint *)(param_1 + 0x10) | 1;
      return 1;
    }
    ERR_ATTIC_error_constprop_0(0x80106,0x456);
    return 0;
  }
  *(uint *)(param_1 + 0x10) = *(uint *)(param_1 + 0x10) & 0xfffffffe;
  return 1;
}



undefined8 file_find(long param_1,undefined8 param_2)

{
  int iVar1;
  undefined8 uVar2;
  
  iVar1 = OSSL_STORE_SEARCH_get_type(param_2);
  if (iVar1 == 1) {
    if (param_1 == 0) {
      return 1;
    }
    if (*(int *)(param_1 + 8) == 2) {
      uVar2 = OSSL_STORE_SEARCH_get0_name(param_2);
      uVar2 = X509_NAME_hash_ex(uVar2,0,0,0);
      BIO_snprintf((char *)(param_1 + 0x24),9,"%08lx",uVar2);
      return 1;
    }
    ERR_ATTIC_error_constprop_0(0x6a,0x478);
  }
  else if (param_1 != 0) {
    ERR_ATTIC_error_constprop_0(0x6d,0x484);
  }
  return 0;
}



char * file_get_pass(UI_METHOD *param_1,char *param_2,int param_3,char *param_4,char *param_5,
                    void *param_6)

{
  int iVar1;
  UI *ui;
  char *prompt;
  
  ui = UI_new();
  if (ui == (UI *)0x0) {
    ERR_ATTIC_error_constprop_0(0x80028,0x3e);
    return (char *)0x0;
  }
  if (param_1 != (UI_METHOD *)0x0) {
    UI_set_method(ui,param_1);
  }
  UI_add_user_data(ui,param_6);
  prompt = UI_construct_prompt(ui,param_4,param_5);
  if (prompt == (char *)0x0) {
    ERR_ATTIC_error_constprop_0(0x80028,0x47);
  }
  else {
    iVar1 = UI_add_input_string(ui,prompt,2,param_2,0,param_3 + -1);
    if (iVar1 < 1) {
      ERR_ATTIC_error_constprop_0(0x80028,0x4b);
    }
    else {
      iVar1 = UI_process(ui);
      if (iVar1 == -2) {
        ERR_ATTIC_error_constprop_0(0x6b,0x50);
      }
      else {
        if (iVar1 != -1) goto LAB_00101152;
        ERR_ATTIC_error_constprop_0(0x80028,0x54);
      }
    }
  }
  param_2 = (char *)0x0;
LAB_00101152:
  CRYPTO_free(prompt);
  UI_free(ui);
  return param_2;
}



size_t file_get_pem_pass(undefined8 param_1,int param_2,undefined8 param_3,undefined8 *param_4)

{
  char *__s;
  size_t sVar1;
  
  __s = (char *)file_get_pass(*param_4,param_1,(long)param_2,param_4[2],param_4[3],param_4[1]);
  sVar1 = 0;
  if (__s != (char *)0x0) {
    sVar1 = strlen(__s);
  }
  return sVar1;
}



long try_decode_PKCS8Encrypted
               (char *param_1,undefined8 param_2,uchar *param_3,long param_4,undefined8 param_5,
               undefined4 *param_6,undefined8 param_7,undefined8 param_8,undefined8 param_9)

{
  int *piVar1;
  int iVar2;
  X509_SIG *a;
  BUF_MEM *a_00;
  char *pcVar3;
  size_t sVar4;
  uchar *puVar5;
  undefined8 *ptr;
  long lVar6;
  long in_FS_OFFSET;
  uchar *local_460;
  int local_454;
  X509_ALGOR *local_450;
  int *local_448;
  uchar *local_440;
  undefined1 local_438 [1032];
  long local_30;
  
  local_30 = *(long *)(in_FS_OFFSET + 0x28);
  local_450 = (X509_ALGOR *)0x0;
  local_448 = (int *)0x0;
  local_440 = (uchar *)0x0;
  local_460 = param_3;
  if (param_1 == (char *)0x0) {
LAB_001012a3:
    a = d2i_X509_SIG((X509_SIG **)0x0,&local_460,param_4);
    if (a != (X509_SIG *)0x0) {
      *param_6 = 1;
      a_00 = BUF_MEM_new();
      if (a_00 == (BUF_MEM *)0x0) {
        ERR_ATTIC_error_constprop_0(0x80007,0x1cd);
      }
      else {
        pcVar3 = (char *)file_get_pass(param_7,local_438,0x400,"PKCS8 decrypt pass phrase",param_9,
                                       param_8);
        if (pcVar3 == (char *)0x0) {
          ERR_ATTIC_error_constprop_0(0x65,0x1d4);
        }
        else {
          X509_SIG_get0(a,&local_450,&local_448);
          piVar1 = local_448;
          sVar4 = strlen(pcVar3);
          puVar5 = PKCS12_pbe_crypt(local_450,pcVar3,(int)sVar4,*(uchar **)(piVar1 + 2),*piVar1,
                                    &local_440,&local_454,0);
          if (puVar5 != (uchar *)0x0) {
            a_00->data = (char *)local_440;
            a_00->length = (long)local_454;
            a_00->max = (long)local_454;
            X509_SIG_free(a);
            ptr = (undefined8 *)CRYPTO_zalloc(0x10,"engines/e_loader_attic.c",0xc1);
            if (ptr != (undefined8 *)0x0) {
              lVar6 = OSSL_STORE_INFO_new(0xffffffff,ptr);
              if (lVar6 == 0) {
                ERR_ATTIC_error_constprop_0(0x8002c,0xc4);
                CRYPTO_free(ptr);
              }
              else {
                *ptr = a_00;
                pcVar3 = CRYPTO_strdup("PRIVATE KEY","engines/e_loader_attic.c",0xcb);
                ptr[1] = pcVar3;
                if (pcVar3 != (char *)0x0) goto LAB_001013ce;
                store_info_free(lVar6);
              }
            }
            a = (X509_SIG *)0x0;
            ERR_ATTIC_error_constprop_0(0x8002c,0x1e4);
          }
        }
      }
      X509_SIG_free(a);
      BUF_MEM_free(a_00);
    }
  }
  else {
    iVar2 = strcmp(param_1,"ENCRYPTED PRIVATE KEY");
    if (iVar2 == 0) {
      *param_6 = 1;
      goto LAB_001012a3;
    }
  }
  lVar6 = 0;
LAB_001013ce:
  if (local_30 == *(long *)(in_FS_OFFSET + 0x28)) {
    return lVar6;
  }
                    /* WARNING: Subroutine does not return */
  __stack_chk_fail();
}



undefined8
try_decode_PKCS12(long param_1,undefined8 param_2,uchar *param_3,long param_4,long *param_5,
                 undefined4 *param_6,undefined8 param_7,undefined8 param_8,undefined8 param_9)

{
  int iVar1;
  undefined8 uVar2;
  PKCS12 *p12;
  size_t sVar3;
  long lVar4;
  long lVar5;
  undefined8 uVar6;
  long lVar7;
  char *__s;
  long lVar8;
  long in_FS_OFFSET;
  long local_488;
  uchar *local_470 [2];
  EVP_PKEY *local_460;
  X509 *local_458;
  stack_st_X509 *local_450;
  undefined1 local_448 [1032];
  long local_40;
  
  lVar4 = *param_5;
  local_40 = *(long *)(in_FS_OFFSET + 0x28);
  local_470[0] = param_3;
  if (lVar4 != 0) {
LAB_001014e4:
    *param_6 = 1;
    uVar2 = OPENSSL_sk_shift(lVar4);
    goto LAB_001014f6;
  }
  uVar2 = 0;
  if (param_1 != 0) goto LAB_001014f6;
  p12 = d2i_PKCS12((PKCS12 **)0x0,local_470,param_4);
  if (p12 == (PKCS12 *)0x0) {
    PKCS12_free((PKCS12 *)0x0);
    goto LAB_001014f6;
  }
  *param_6 = 1;
  local_460 = (EVP_PKEY *)0x0;
  local_458 = (X509 *)0x0;
  local_450 = (stack_st_X509 *)0x0;
  iVar1 = PKCS12_mac_present();
  if (iVar1 == 0) {
LAB_0010157a:
    __s = "";
LAB_00101581:
    iVar1 = PKCS12_parse(p12,__s,&local_460,&local_458,&local_450);
    if (iVar1 == 0) {
      PKCS12_free(p12);
      goto LAB_001014f6;
    }
    lVar4 = OPENSSL_sk_new_null();
    if (lVar4 != 0) {
      lVar8 = 0;
      if (local_460 == (EVP_PKEY *)0x0) {
LAB_001016b9:
        if (local_458 != (X509 *)0x0) {
          lVar5 = OSSL_STORE_INFO_new_CERT();
          if (lVar5 == 0) goto LAB_001017da;
          local_458 = (X509 *)0x0;
          iVar1 = OPENSSL_sk_push(lVar4,lVar5);
          if (iVar1 == 0) goto LAB_001017dc;
        }
        local_488 = 0;
        do {
          iVar1 = OPENSSL_sk_num(local_450);
          if (iVar1 < 1) {
            EVP_PKEY_free(local_460);
            X509_free(local_458);
            OSSL_STACK_OF_X509_free(local_450);
            store_info_free(0);
            store_info_free(0);
            store_info_free(0);
            *param_5 = lVar4;
            PKCS12_free(p12);
            goto LAB_001014e4;
          }
          uVar6 = OPENSSL_sk_value(local_450,0);
          lVar7 = OSSL_STORE_INFO_new_CERT(uVar6);
          if (lVar7 == 0) goto LAB_00101847;
          lVar5 = OPENSSL_sk_shift(local_450);
          if (lVar5 == 0) break;
          iVar1 = OPENSSL_sk_push(lVar4);
        } while (iVar1 != 0);
        lVar5 = 0;
        lVar8 = lVar7;
      }
      else {
        local_488 = OSSL_STORE_INFO_new_PKEY();
        if (local_488 == 0) {
LAB_001017da:
          lVar5 = 0;
LAB_001017dc:
          local_488 = 0;
        }
        else {
          local_460 = (EVP_PKEY *)0x0;
          iVar1 = OPENSSL_sk_push(lVar4,local_488);
          if (iVar1 != 0) goto LAB_001016b9;
LAB_00101847:
          lVar5 = 0;
        }
      }
      EVP_PKEY_free(local_460);
      X509_free(local_458);
      OSSL_STACK_OF_X509_free(local_450);
      store_info_free(local_488);
      store_info_free(lVar5);
      store_info_free(lVar8);
      OPENSSL_sk_pop_free(lVar4,store_info_free);
      *param_5 = 0;
      PKCS12_free(p12);
      goto LAB_001014f6;
    }
    EVP_PKEY_free(local_460);
    X509_free(local_458);
    OSSL_STACK_OF_X509_free(local_450);
    store_info_free(0);
    store_info_free(0);
    store_info_free(0);
    *param_5 = 0;
  }
  else {
    iVar1 = PKCS12_verify_mac(p12,"",0);
    if (iVar1 != 0) goto LAB_0010157a;
    iVar1 = PKCS12_verify_mac(p12,(char *)0x0,0);
    if (iVar1 != 0) goto LAB_0010157a;
    __s = (char *)file_get_pass(param_7,local_448,0x400,"PKCS12 import",param_9,param_8);
    if (__s != (char *)0x0) {
      sVar3 = strlen(__s);
      iVar1 = PKCS12_verify_mac(p12,__s,(int)sVar3);
      if (iVar1 == 0) {
        ERR_ATTIC_error_constprop_0(0x66,0x14f);
        goto LAB_00101658;
      }
      goto LAB_00101581;
    }
    ERR_ATTIC_error_constprop_0(0x68,0x14b);
  }
LAB_00101658:
  PKCS12_free(p12);
LAB_001014f6:
  if (local_40 == *(long *)(in_FS_OFFSET + 0x28)) {
    return uVar2;
  }
                    /* WARNING: Subroutine does not return */
  __stack_chk_fail();
}



ulong file_eof(long param_1)

{
  ulong uVar1;
  
  if (*(int *)(param_1 + 8) == 2) {
    return (ulong)*(uint *)(param_1 + 0x20);
  }
  if ((*(long *)(param_1 + 0x20) != 0) &&
     (uVar1 = (**(code **)(*(long *)(param_1 + 0x20) + 0x10))(*(undefined8 *)(param_1 + 0x28)),
     (int)uVar1 == 0)) {
    return uVar1;
  }
  uVar1 = BIO_ctrl(*(BIO **)(param_1 + 0x18),2,0,(void *)0x0);
  return uVar1;
}



undefined8 loader_attic_finish(void)

{
  return 1;
}



long file_load(undefined8 *param_1,size_t *param_2,undefined8 param_3)

{
  byte *pbVar1;
  byte bVar2;
  BIO *pBVar3;
  undefined8 uVar4;
  char *__s;
  int iVar5;
  int iVar6;
  long lVar7;
  EVP_PKEY *pEVar8;
  size_t sVar9;
  long *ptr;
  ulong uVar10;
  undefined1 (*pauVar11) [16];
  ushort **ppuVar12;
  size_t sVar13;
  void *ptr_00;
  int *piVar14;
  undefined **ppuVar15;
  byte *pbVar16;
  char *pcVar17;
  undefined *puVar18;
  long in_FS_OFFSET;
  long local_140;
  uchar *local_138;
  size_t local_130;
  char *local_120;
  char *local_e8;
  BUF_MEM *local_e0;
  int local_cc;
  undefined4 local_c8;
  int local_c4;
  char *local_c0;
  char *local_b8;
  uchar *local_b0;
  size_t local_a8;
  size_t *local_a0;
  size_t *local_98;
  undefined8 local_90;
  char *local_88;
  undefined8 local_80;
  EVP_CIPHER_INFO local_78;
  undefined1 local_58 [16];
  undefined8 local_48;
  long local_40;
  
  local_40 = *(long *)(in_FS_OFFSET + 0x28);
  *(undefined4 *)((long)param_1 + 0xc) = 0;
  if (*(int *)(param_1 + 1) != 2) {
    do {
      local_98 = (size_t *)((ulong)local_98 & 0xffffffff00000000);
      if (param_1[4] != 0) {
        local_140 = (**(code **)(param_1[4] + 8))
                              (0,0,0,0,param_1 + 5,&local_98,param_2,param_3,*param_1,param_1[9],
                               param_1[10]);
        if (local_140 != 0) goto LAB_00101974;
        (**(code **)(param_1[4] + 0x18))(param_1 + 5);
        *(undefined1 (*) [16])(param_1 + 4) = (undefined1  [16])0x0;
      }
      iVar5 = file_eof(param_1);
      if (iVar5 != 0) goto LAB_00101bff;
      local_140 = 0;
LAB_00101a64:
      pBVar3 = (BIO *)param_1[3];
      local_c0 = (char *)0x0;
      local_b8 = (char *)0x0;
      local_b0 = (uchar *)0x0;
      local_a8 = 0;
      if (*(int *)(param_1 + 1) == 1) {
        uVar4 = *param_1;
        if ((*(byte *)(param_1 + 2) & 1) != 0) {
          iVar5 = PEM_read_bio_ex(pBVar3);
          if (iVar5 < 1) goto LAB_001020f1;
LAB_00101c57:
          pcVar17 = local_b8;
          iVar5 = -1;
          local_120 = local_b8;
          sVar9 = strlen(local_b8);
          if (10 < sVar9) {
            iVar6 = PEM_get_EVP_CIPHER_INFO(pcVar17,&local_78);
            if (iVar6 == 0) goto LAB_001020f1;
            local_88 = "PEM pass phrase";
            local_98 = param_2;
            local_90 = param_3;
            local_80 = uVar4;
            iVar6 = PEM_do_header(&local_78,local_b0,(long *)&local_a8,file_get_pem_pass,&local_98);
            if (iVar6 == 0) goto LAB_001020f1;
            goto LAB_0010217d;
          }
          goto LAB_00101c7c;
        }
        iVar5 = PEM_read_bio(pBVar3,&local_c0,&local_b8,&local_b0,(long *)&local_a8);
        if (0 < iVar5) goto LAB_00101c57;
LAB_001020f1:
        *(int *)((long)param_1 + 0xc) = *(int *)((long)param_1 + 0xc) + 1;
        iVar5 = -1;
      }
      else {
        local_cc = -1;
        local_c8 = 0;
        local_c4 = 0;
        local_a0 = (size_t *)((ulong)local_a0 & 0xffffffff00000000);
        local_58 = (undefined1  [16])0x0;
        local_98 = (size_t *)local_58;
        lVar7 = BIO_ctrl(pBVar3,0x1d,0x10,local_58);
        if ((lVar7 < 1) ||
           (iVar5 = ossl_do_blob_header(&local_98,0x10,&local_c8,&local_c4,&local_a0,&local_cc),
           iVar5 < 1)) {
          iVar5 = -1;
        }
        else {
          if (local_cc != 0) {
            pEVar8 = b2i_PublicKey_bio(pBVar3);
            if (pEVar8 == (EVP_PKEY *)0x0) goto LAB_00101fbc;
LAB_00101b65:
            local_140 = OSSL_STORE_INFO_new_PKEY(pEVar8);
            if (local_140 == 0) goto LAB_00101fbc;
            iVar5 = 0;
            goto LAB_00101b86;
          }
          pEVar8 = b2i_PrivateKey_bio(pBVar3);
          if (pEVar8 != (EVP_PKEY *)0x0) goto LAB_00101b65;
LAB_00101fbc:
          iVar5 = 0;
          EVP_PKEY_free(pEVar8);
        }
        pBVar3 = (BIO *)param_1[3];
        uVar4 = *param_1;
        local_c8 = 0;
        local_c4 = 0;
        local_a0 = (size_t *)CONCAT44(local_a0._4_4_,0xffffffff);
        local_48 = 0;
        local_58 = (undefined1  [16])0x0;
        local_98 = (size_t *)local_58;
        lVar7 = BIO_ctrl(pBVar3,0x1d,0x18,local_58);
        if ((0 < lVar7) &&
           (iVar6 = ossl_do_PVK_header(&local_98,0x18,0,&local_a0,&local_c8,&local_c4), iVar6 != 0))
        {
          iVar5 = iVar5 + 1;
          local_88 = "PVK pass phrase";
          local_98 = param_2;
          local_90 = param_3;
          local_80 = uVar4;
          pEVar8 = b2i_PVK_bio(pBVar3,file_get_pem_pass,&local_98);
          if ((pEVar8 != (EVP_PKEY *)0x0) &&
             (local_140 = OSSL_STORE_INFO_new_PKEY(pEVar8), local_140 != 0)) goto LAB_00101b86;
          EVP_PKEY_free(pEVar8);
        }
        local_98 = (size_t *)0x0;
        iVar6 = asn1_d2i_read_bio(param_1[3],&local_98);
        if (iVar6 < 0) {
          *(int *)((long)param_1 + 0xc) = *(int *)((long)param_1 + 0xc) + 1;
          local_140 = 0;
        }
        else {
          local_b0 = (uchar *)local_98[1];
          local_a8 = *local_98;
          CRYPTO_free(local_98);
LAB_0010217d:
          local_120 = local_b8;
LAB_00101c7c:
          local_e8 = (char *)0x0;
          local_e0 = (BUF_MEM *)0x0;
          local_130 = local_a8;
          local_138 = local_b0;
          pcVar17 = local_c0;
          while( true ) {
            local_a0 = (size_t *)0x0;
            ptr = (long *)CRYPTO_zalloc(0x38,"engines/e_loader_attic.c",0x499);
            if (ptr == (long *)0x0) break;
            ppuVar15 = &file_handlers;
            iVar5 = 0;
            local_140 = 0;
            do {
              local_c4 = 0;
              puVar18 = *ppuVar15;
              local_98 = (size_t *)0x0;
              ERR_set_mark();
              lVar7 = (**(code **)(puVar18 + 8))
                                (pcVar17,local_120,local_138,local_130,&local_98,&local_c4,param_2,
                                 param_3,*param_1,param_1[9],param_1[10]);
              uVar10 = ERR_peek_last_error();
              if ((((uVar10 & 0x80000000) == 0) && ((char)(uVar10 >> 0x17) == '\r')) &&
                 (((uint)uVar10 & 0x7fffff) == 0x8010a)) {
                ERR_pop_to_mark();
              }
              else {
                ERR_clear_last_mark();
              }
              if (0 < local_c4) {
                ptr[iVar5] = (long)puVar18;
                if (local_a0 != (size_t *)0x0) {
                  (**(code **)(puVar18 + 0x18))(&local_a0);
                }
                iVar5 = iVar5 + local_c4;
                local_a0 = local_98;
                if (1 < iVar5) {
                  store_info_free(local_140);
                  store_info_free(lVar7);
                  if (*(code **)(puVar18 + 0x18) != (code *)0x0) {
                    (**(code **)(puVar18 + 0x18))(&local_a0);
                  }
                  local_a0 = (size_t *)0x0;
                  goto LAB_00101e58;
                }
                if ((local_140 == 0) && (local_140 = lVar7, lVar7 == 0)) goto LAB_00101e58;
              }
              ppuVar15 = ppuVar15 + 1;
            } while (ppuVar15 != (undefined **)0x102b38);
            if (local_140 == 0) {
LAB_00101e58:
              CRYPTO_free(ptr);
              CRYPTO_free(local_e8);
              BUF_MEM_free(local_e0);
              goto LAB_00101e8b;
            }
            if ((iVar5 == 1) && (*(int *)(*ptr + 0x20) != 0)) {
              param_1[4] = *ptr;
              param_1[5] = local_a0;
            }
            CRYPTO_free(ptr);
            CRYPTO_free(local_e8);
            BUF_MEM_free(local_e0);
            iVar6 = OSSL_STORE_INFO_get_type(local_140);
            if (iVar6 != -1) goto LAB_00101b86;
            pauVar11 = (undefined1 (*) [16])OSSL_STORE_INFO_get0_data(0xffffffff,local_140);
            local_e0 = *(BUF_MEM **)*pauVar11;
            pcVar17 = *(char **)(*pauVar11 + 8);
            local_138 = (uchar *)local_e0->data;
            local_130 = local_e0->length;
            *pauVar11 = (undefined1  [16])0x0;
            store_info_free(local_140);
            local_e8 = pcVar17;
          }
          CRYPTO_free(local_e8);
          BUF_MEM_free(local_e0);
LAB_00101e8b:
          if (local_c0 == (char *)0x0) {
            if (iVar5 < 2) goto LAB_001021d0;
            ERR_ATTIC_error_constprop_0(100,0x63d);
          }
          else if (iVar5 < 2) {
LAB_001021d0:
            local_140 = 0;
            if (iVar5 != 1) goto LAB_00101b86;
            uVar10 = ERR_peek_error();
            if ((uVar10 == 0) && (ERR_ATTIC_error_constprop_0(0x6c,0x644), local_c0 != (char *)0x0))
            {
              ERR_add_error_data(3,"PEM type is \'",local_c0);
            }
          }
          *(int *)((long)param_1 + 0xc) = *(int *)((long)param_1 + 0xc) + 1;
          local_140 = 0;
        }
      }
LAB_00101b86:
      if ((*(byte *)(param_1 + 2) & 1) == 0) {
        CRYPTO_free(local_c0);
        if ((*(byte *)(param_1 + 2) & 1) == 0) goto LAB_00101bb3;
LAB_00101a04:
        CRYPTO_secure_clear_free(local_b8,0,"engines/e_loader_attic.c");
      }
      else {
        CRYPTO_secure_clear_free(local_c0,0,"engines/e_loader_attic.c");
        if ((*(byte *)(param_1 + 2) & 1) != 0) goto LAB_00101a04;
LAB_00101bb3:
        CRYPTO_free(local_b8);
      }
      if ((*(byte *)(param_1 + 2) & 1) == 0) {
        CRYPTO_free(local_b0);
      }
      else {
        CRYPTO_secure_clear_free(local_b0,local_a8,"engines/e_loader_attic.c");
      }
      if (iVar5 == 0) {
        iVar5 = file_eof(param_1);
        if ((iVar5 != 0) || (0 < *(int *)((long)param_1 + 0xc))) goto LAB_00102194;
        goto LAB_00101a64;
      }
      if (1 < iVar5) {
        store_info_free(local_140);
        goto LAB_00101bff;
      }
LAB_00102194:
      if (local_140 == 0) goto LAB_00101bff;
      iVar5 = *(int *)(param_1 + 8);
      if ((iVar5 == 0) || (iVar6 = OSSL_STORE_INFO_get_type(local_140), iVar5 == iVar6))
      goto LAB_00101974;
      store_info_free();
    } while( true );
  }
  pcVar17 = (char *)param_1[6];
  do {
    if (pcVar17 == (char *)0x0) {
      if (*(int *)(param_1 + 4) == 0) {
        ERR_new();
        ERR_set_debug("engines/e_loader_attic.c",0x5e9,"file_load");
        ERR_set_error(2,*(undefined4 *)(param_1 + 7),0);
        *(int *)((long)param_1 + 0xc) = *(int *)((long)param_1 + 0xc) + 1;
      }
LAB_00101bff:
      local_140 = 0;
LAB_00101974:
      if (local_40 == *(long *)(in_FS_OFFSET + 0x28)) {
        return local_140;
      }
                    /* WARNING: Subroutine does not return */
      __stack_chk_fail();
    }
    if (*pcVar17 != '.') {
      sVar9 = strlen((char *)((long)param_1 + 0x24));
      if (*(char *)((long)param_1 + 0x24) == '\0') {
LAB_00102394:
        __s = (char *)*param_1;
        sVar9 = strlen(__s);
        if ((*__s == '\0') || (__s[sVar9 - 1] != '/')) {
          sVar9 = sVar9 + 1;
          puVar18 = &_LC20;
        }
        else {
          puVar18 = &_LC18;
        }
        sVar13 = strlen(pcVar17);
        lVar7 = sVar9 + 1 + sVar13;
        ptr_00 = (void *)CRYPTO_zalloc(lVar7,"engines/e_loader_attic.c",0x58f);
        if (ptr_00 == (void *)0x0) goto LAB_00101bff;
        OPENSSL_strlcat(ptr_00,*param_1,lVar7);
        OPENSSL_strlcat(ptr_00,puVar18,lVar7);
        OPENSSL_strlcat(ptr_00,pcVar17,lVar7);
        lVar7 = OPENSSL_DIR_read(param_1 + 3,*param_1);
        param_1[6] = lVar7;
        piVar14 = __errno_location();
        iVar5 = *piVar14;
        *(int *)(param_1 + 7) = iVar5;
        if ((iVar5 == 0) && (lVar7 == 0)) {
          *(undefined4 *)(param_1 + 4) = 1;
        }
        local_140 = OSSL_STORE_INFO_new_NAME(ptr_00);
        if (local_140 == 0) {
          CRYPTO_free(ptr_00);
          ERR_ATTIC_error_constprop_0(0x8002c,0x602);
        }
        goto LAB_00101974;
      }
      if ((((*(int *)(param_1 + 8) - 5U < 2) || (*(int *)(param_1 + 8) == 0)) &&
          (iVar5 = OPENSSL_strncasecmp(pcVar17,(char *)((long)param_1 + 0x24),sVar9), iVar5 == 0))
         && (pcVar17[sVar9] == '.')) {
        pbVar16 = (byte *)(pcVar17 + sVar9 + 1);
        iVar5 = *(int *)(param_1 + 8);
        bVar2 = *pbVar16;
        if (bVar2 == 0x72) {
          if ((iVar5 == 0) || (iVar5 == 6)) {
            bVar2 = pbVar16[1];
            pbVar16 = pbVar16 + 1;
            goto LAB_00102365;
          }
        }
        else if (iVar5 != 6) {
LAB_00102365:
          ppuVar12 = __ctype_b_loc();
          if ((*(byte *)((long)*ppuVar12 + (ulong)bVar2 * 2 + 1) & 8) != 0) {
            do {
              pbVar1 = pbVar16 + 1;
              pbVar16 = pbVar16 + 1;
            } while ((*(byte *)((long)*ppuVar12 + (ulong)*pbVar1 * 2 + 1) & 8) != 0);
            if (*pbVar1 == 0) {
              pcVar17 = (char *)param_1[6];
              goto LAB_00102394;
            }
          }
        }
      }
    }
    lVar7 = OPENSSL_DIR_read(param_1 + 3,*param_1);
    param_1[6] = lVar7;
    piVar14 = __errno_location();
    iVar5 = *piVar14;
    *(int *)(param_1 + 7) = iVar5;
    if ((iVar5 == 0) && (lVar7 == 0)) {
      *(undefined4 *)(param_1 + 4) = 1;
    }
    iVar5 = file_eof(param_1);
    if (iVar5 != 0) goto LAB_00101bff;
    pcVar17 = (char *)param_1[6];
  } while( true );
}



uint v_check(ulong param_1)

{
  return ~-(uint)(param_1 < 0x30000) & 0x30000;
}



undefined8 bind_engine(ENGINE *param_1,char *param_2,long *param_3)

{
  int iVar1;
  void *pvVar2;
  
  pvVar2 = ENGINE_get_static_state();
  if ((void *)*param_3 != pvVar2) {
    CRYPTO_set_mem_functions((m *)param_3[1],(r *)param_3[2],(f *)param_3[3]);
    OPENSSL_init_crypto(0x80000,0);
  }
  if ((param_2 == (char *)0x0) || (iVar1 = strcmp(param_2,"loader_attic"), iVar1 == 0)) {
    if (lib_code == 0) {
      lib_code = ERR_get_next_error_library();
    }
    if (error_loaded == 0) {
      ERR_load_strings(lib_code,(ERR_STRING_DATA *)ATTIC_str_reasons);
      error_loaded = 1;
    }
    loader_attic = OSSL_STORE_LOADER_new(param_1,&_LC0);
    if (((((((loader_attic != 0) &&
            (iVar1 = OSSL_STORE_LOADER_set_open_ex(loader_attic,file_open_ex), iVar1 != 0)) &&
           (iVar1 = OSSL_STORE_LOADER_set_open(loader_attic,file_open), iVar1 != 0)) &&
          ((iVar1 = OSSL_STORE_LOADER_set_attach(loader_attic,file_attach), iVar1 != 0 &&
           (iVar1 = OSSL_STORE_LOADER_set_ctrl(loader_attic,file_ctrl), iVar1 != 0)))) &&
         ((iVar1 = OSSL_STORE_LOADER_set_expect(loader_attic,0x100000), iVar1 != 0 &&
          ((iVar1 = OSSL_STORE_LOADER_set_find(loader_attic,file_find), iVar1 != 0 &&
           (iVar1 = OSSL_STORE_LOADER_set_load(loader_attic,file_load), iVar1 != 0)))))) &&
        (iVar1 = OSSL_STORE_LOADER_set_eof(loader_attic,file_eof), iVar1 != 0)) &&
       ((((iVar1 = OSSL_STORE_LOADER_set_error(loader_attic,file_error), iVar1 != 0 &&
          (iVar1 = OSSL_STORE_LOADER_set_close(loader_attic,file_close), iVar1 != 0)) &&
         (iVar1 = ENGINE_set_id(param_1,"loader_attic"), iVar1 != 0)) &&
        (((iVar1 = ENGINE_set_name(param_1,"\'file:\' loader"), iVar1 != 0 &&
          (iVar1 = ENGINE_set_destroy_function(param_1,loader_attic_destroy), iVar1 != 0)) &&
         ((iVar1 = ENGINE_set_init_function(param_1,loader_attic_init), iVar1 != 0 &&
          ((iVar1 = ENGINE_set_finish_function(param_1,loader_attic_finish), iVar1 != 0 &&
           (iVar1 = OSSL_STORE_register_loader(loader_attic), iVar1 != 0)))))))))) {
      return 1;
    }
    OSSL_STORE_LOADER_free(loader_attic);
    loader_attic = 0;
    ERR_ATTIC_error_constprop_0(0x67,0x6c9);
  }
  return 0;
}


