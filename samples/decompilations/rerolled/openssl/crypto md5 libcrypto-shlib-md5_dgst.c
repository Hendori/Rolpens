int MD5_Update(MD5_CTX *c, void *data, size_t len) {
   ...
   if (uVar3 == 0) {
      ...
      LAB_00100107:ossl_md5_block_asm_data_order(c, data, len >> 6);
...
int MD5_Final(uchar *md, MD5_CTX *c) {
 ...
 while ( uVar6 < uVar3 );
   ...
   LAB_00100314:ossl_md5_block_asm_data_order(c, ptr, 1);
   uVar6 = 0x38;
   puVar4 = ptr;
...
/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */int MD5_Init(MD5_CTX *c) {
   undefined4 uVar1;
   undefined8 uVar2;
   undefined8 uVar3;
   for (int i = 0; i < 4; i++) {
