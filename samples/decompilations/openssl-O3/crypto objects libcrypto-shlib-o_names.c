...

{
  ...
  if (obj_lock != 0) {
    ...
    names_lh = OPENSSL_LH_set_thunks
                         (uVar1,0x100000,lh_OBJ_NAME_cfn_thunk,lh_OBJ_NAME_doall_thunk,
...
void lh_OBJ_NAME_doall_OBJ_DOALL_thunk
...

{
  ...
  iVar4 = OBJ_NAME_init();
  if ((iVar4 == 0) || (iVar4 = CRYPTO_THREAD_write_lock(obj_lock), iVar4 == 0)) {
    iVar7 = 0;
  }
  else {
...

{
  ...
  iVar3 = OBJ_NAME_init();
  if ((iVar3 == 0) ||
     (ptr = (ulong *)CRYPTO_malloc(0x18,"crypto/objects/o_names.c",199), ptr == (ulong *)0x0)) {
    return 0;
  }
  ptr[1] = (ulong)name;
  uVar2 = _LC1;
