...
void unreserved__init(void) {
  if (segment_0x403dd0_Generic64_768.unreserved__got.offset_16) {
    ((cabifunction_89 *) segment_0x403dd0_Generic64_768.unreserved__got.offset_16)();
  }
}
...
struct_76 main(void) {
  ...
  struct_76 _var_3;
  _var_2 = all_badges();
  _var_1 = polynome_table(_undef_generic64_t(), _undef_generic64_t());
...
void unreserved__start(generic64_t argument_0, generic64_t argument_1, struct_68 *argument_2) {
  int32_t _var_0;
...
generic64_t all_badges(void) {
  int32_t _var_0;
  _var_0 = printf((const int8_t *) "We can open every door with badge #%d\n");
  return (uint64_t) _var_0;
}

_ABI(SystemV_x86_64)
struct_84 polynome_table(generic64_t argument_0, generic64_t argument_1) {
  ...
  *((uint8_t **) &_stack) = "P(%d) = %d\n";
  ...
  _var_0 = printf((const int8_t *) "P(%d) = %d\n");
