...

{
  ...
  parse_gnu_standard_options_only(argc,argv,&_LC1,"GNU coreutils",_Version,1,0x100000,uVar2,0);
  if (_optind + 1 < argc) {
    lVar5 = (long)_optind;
    if (argc <= _optind + 2) {
      iVar1 = link(argv[lVar5],argv[lVar5 + 1]);
      if (iVar1 == 0) {
        return 0;
      }
      uVar2 = quotearg_n_style(1,4,argv[_optind]);
      uVar3 = quotearg_n_style(0,4,argv[(long)_optind + 1]);
      argv = (char **)dcgettext(0,"cannot create link %s to %s",5);
      piVar4 = __errno_location();
      error(1,*piVar4,argv,uVar3,uVar2);
      lVar5 = extraout_RDX;
    }
